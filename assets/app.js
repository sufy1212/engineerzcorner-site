try{let fieldError2=function(inputEl,message){var wrap=inputEl.closest(".field")||inputEl.closest(".parentField");if(!wrap)return!1;var err=wrap.querySelector(".field-error");return message?(wrap.classList.add("has-error"),err||(err=document.createElement("div"),err.className="field-error",wrap.appendChild(err)),err.textContent="\u26A0 "+message,!0):(wrap.classList.remove("has-error"),err&&err.remove(),!1)},updateValidationBanner2=function(bannerId,errorCount){var el=document.getElementById(bannerId);el&&(errorCount===0?(el.className="validation-banner ok",el.innerHTML='<span class="vdot"></span><span>All inputs valid \u2014 calculation below is trustworthy.</span>'):(el.className="validation-banner bad",el.innerHTML='<span class="vdot"></span><span>'+errorCount+" input issue"+(errorCount>1?"s":"")+" found \u2014 see highlighted field"+(errorCount>1?"s":"")+" below for what to change.</span>"))};var fieldError=fieldError2,updateValidationBanner=updateValidationBanner2}catch(e){console.error("app.js module #0 error:",e)}try{var NOTES_HTML=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<div class="wrap" id="top">
  <div class="hero">
    <div class="eyebrow">Personal reference \xB7 theory only</div>
...
`}catch(e){console.error("app.js module #1 error:",e)}try{(function(){var catSelect=document.getElementById("noteCatSelect"),newCatWrap=document.getElementById("noteNewCatWrap");if(!catSelect)return;catSelect.addEventListener("change",function(){newCatWrap.style.display=this.value==="__new__"?"":"none"});function jsEscape(s){return String(s).replace(/\\/g,"\\\\").replace(/"/g,'\\"')}document.getElementById("noteGenBtn").addEventListener("click",function(){var title=document.getElementById("noteTitle").value.trim(),summary=document.getElementById("noteSummary").value.trim(),bodyLines=document.getElementById("noteBody").value.split(`
`).map(function(l){return l.trim()}).filter(Boolean),cat=catSelect.value,newCat=document.getElementById("noteNewCat").value.trim(),out=document.getElementById("noteOutput");if(!title||!summary||bodyLines.length===0){out.value="Fill in title, summary, and at least one body paragraph first.";return}var noteObj=`        {
...
`)+`
        ...
        }`;if(cat==="__new__"&&newCat){var slug=newCat.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"");out.value=`// New subject block \u2014 add this as a new entry in the DATA array (top level, alongside "fundamentals" etc.):
    {
      id:'`+slug+"', label:'"+jsEscape(newCat)+`', color:'#5FD3E3',
      notes:[
`+noteObj+`
      ]
    }`}else out.value='// Paste this inside the "'+cat+`" subject's notes:[ ... ] array, as a new item:
`+noteObj+","}),document.getElementById("noteCopyBtn").addEventListener("click",function(){var out=document.getElementById("noteOutput"),statusEl=document.getElementById("noteCopyStatus");if(!out.value){statusEl.textContent="Generate the code first.";return}out.select(),navigator.clipboard.writeText(out.value).then(function(){statusEl.textContent="Copied \u2014 paste it into the DATA array in the Notes page source."}).catch(function(){statusEl.textContent="Couldn't auto-copy \u2014 the text is selected, use Ctrl/Cmd+C."})});var frame=document.getElementById("notesFrame");frame.srcdoc=NOTES_HTML})()}catch(e){console.error("app.js module #2 error:",e)}try{var MECH_NOTES_HTML=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
`}catch(e){console.error("app.js module #3 error:",e)}try{document.addEventListener("DOMContentLoaded",function(){var catSelect=document.getElementById("mechNoteCatSelect"),newCatWrap=document.getElementById("mechNoteNewCatWrap");if(!catSelect)return;catSelect.addEventListener("change",function(){newCatWrap.style.display=this.value==="__new__"?"":"none"});function jsEscape(s){return String(s).replace(/\\/g,"\\\\").replace(/"/g,'\\"')}document.getElementById("mechNoteGenBtn").addEventListener("click",function(){var title=document.getElementById("mechNoteTitle").value.trim(),summary=document.getElementById("mechNoteSummary").value.trim(),bodyLines=document.getElementById("mechNoteBody").value.split(`
`).map(function(l){return l.trim()}).filter(Boolean),cat=catSelect.value,newCat=document.getElementById("mechNoteNewCat").value.trim(),out=document.getElementById("mechNoteOutput");if(!title||!summary||bodyLines.length===0){out.value="Fill in title, summary, and at least one body paragraph first.";return}var noteObj=`        {
          title:"`+jsEscape(title)+`",
          summary:"`+jsEscape(summary)+`",
          body:[
`+bodyLines.map(function(l){return'            "'+jsEscape(l)+'"'}).join(`,
...
`+noteObj+","}),document.getElementById("mechNoteCopyBtn").addEventListener("click",function(){var out=document.getElementById("mechNoteOutput"),statusEl=document.getElementById("mechNoteCopyStatus");if(!out.value){statusEl.textContent="Generate the code first.";return}out.select(),navigator.clipboard.writeText(out.value).then(function(){statusEl.textContent="Copied \u2014 paste it into the DATA array in the Notes page source."}).catch(function(){statusEl.textContent="Couldn't auto-copy \u2014 the text is selected, use Ctrl/Cmd+C."})});var mechFrame=document.getElementById("mechNotesFrame");mechFrame&&(mechFrame.srcdoc=MECH_NOTES_HTML)})}catch(e){console.error("app.js module #4 error:",e)}try{var HVAC_NOTES_HTML=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
`}catch(e){console.error("app.js module #5 error:",e)}try{document.addEventListener("DOMContentLoaded",function(){var catSelect=document.getElementById("hvacNoteCatSelect"),newCatWrap=document.getElementById("hvacNoteNewCatWrap");if(!catSelect)return;catSelect.addEventListener("change",function(){newCatWrap.style.display=this.value==="__new__"?"":"none"});function jsEscape(s){return String(s).replace(/\\/g,"\\\\").replace(/"/g,'\\"')}document.getElementById("hvacNoteGenBtn").addEventListener("click",function(){var title=document.getElementById("hvacNoteTitle").value.trim(),summary=document.getElementById("hvacNoteSummary").value.trim(),bodyLines=document.getElementById("hvacNoteBody").value.split(`
`).map(function(l){return l.trim()}).filter(Boolean),cat=catSelect.value,newCat=document.getElementById("hvacNoteNewCat").value.trim(),out=document.getElementById("hvacNoteOutput");if(!title||!summary||bodyLines.length===0){out.value="Fill in title, summary, and at least one body paragraph first.";return}var noteObj=`        {
          title:"`+jsEscape(title)+`",
          summary:"`+jsEscape(summary)+`",
          body:[
`+bodyLines.map(function(l){return'            "'+jsEscape(l)+'"'}).join(`,
`)+`
        ...
        }`;if(cat==="__new__"&&newCat){var slug=newCat.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"");out.value=`// New subject block \u2014 add this as a new entry in the DATA array (top level, alongside "psychrometrics" etc.):
    {
...
`+noteObj+","}),document.getElementById("hvacNoteCopyBtn").addEventListener("click",function(){var out=document.getElementById("hvacNoteOutput"),statusEl=document.getElementById("hvacNoteCopyStatus");if(!out.value){statusEl.textContent="Generate the code first.";return}out.select(),navigator.clipboard.writeText(out.value).then(function(){statusEl.textContent="Copied \u2014 paste it into the DATA array in the Notes page source."}).catch(function(){statusEl.textContent="Couldn't auto-copy \u2014 the text is selected, use Ctrl/Cmd+C."})});var hvacFrame=document.getElementById("hvacNotesFrame");hvacFrame&&(hvacFrame.srcdoc=HVAC_NOTES_HTML)})}catch(e){console.error("app.js module #6 error:",e)}try{var AUTO_NOTES_HTML=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
`}catch(e){console.error("app.js module #7 error:",e)}try{document.addEventListener("DOMContentLoaded",function(){var catSelect=document.getElementById("autoNoteCatSelect"),newCatWrap=document.getElementById("autoNoteNewCatWrap");if(!catSelect)return;catSelect.addEventListener("change",function(){newCatWrap.style.display=this.value==="__new__"?"":"none"});function jsEscape(s){return String(s).replace(/\\/g,"\\\\").replace(/"/g,'\\"')}document.getElementById("autoNoteGenBtn").addEventListener("click",function(){var title=document.getElementById("autoNoteTitle").value.trim(),summary=document.getElementById("autoNoteSummary").value.trim(),bodyLines=document.getElementById("autoNoteBody").value.split(`
`).map(function(l){return l.trim()}).filter(Boolean),cat=catSelect.value,newCat=document.getElementById("autoNoteNewCat").value.trim(),out=document.getElementById("autoNoteOutput");if(!title||!summary||bodyLines.length===0){out.value="Fill in title, summary, and at least one body paragraph first.";return}var noteObj=`        {
          title:"`+jsEscape(title)+`",
          summary:"`+jsEscape(summary)+`",
          body:[
`+bodyLines.map(function(l){return'            "'+jsEscape(l)+'"'}).join(`,
`)+`
        ...
        }`;if(cat==="__new__"&&newCat){var slug=newCat.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"");out.value=`// New subject block \u2014 add this as a new entry in the DATA array (top level, alongside "control-theory" etc.):
    {
...
`+noteObj+","}),document.getElementById("autoNoteCopyBtn").addEventListener("click",function(){var out=document.getElementById("autoNoteOutput"),statusEl=document.getElementById("autoNoteCopyStatus");if(!out.value){statusEl.textContent="Generate the code first.";return}out.select(),navigator.clipboard.writeText(out.value).then(function(){statusEl.textContent="Copied \u2014 paste it into the DATA array in the Notes page source."}).catch(function(){statusEl.textContent="Couldn't auto-copy \u2014 the text is selected, use Ctrl/Cmd+C."})});var autoFrame=document.getElementById("autoNotesFrame");autoFrame&&(autoFrame.srcdoc=AUTO_NOTES_HTML)})}catch(e){console.error("app.js module #8 error:",e)}try{(function(){var TOOLS=[{key:"sc",page:"page-sc",list:"dbList",addBtn:"addDbBtn",name:"short-circuit"},{key:"vd",page:"page-vd",list:"vdList",addBtn:"vdAddBtn",name:"voltage-drop"},{key:"sel",page:"page-sel",list:"selList",addBtn:"selAddBtn",name:"selectivity"},{key:"cs",page:"page-cs",list:"csList",addBtn:"csAddBtn",name:"cable-sizing"},{key:"est",page:"page-est",list:"estOutList",addBtn:"estOutAddBtn",name:"panel-estimate"},{key:"tr",page:"page-tr",list:"trsizeList",addBtn:"trsizeAddBtn",name:"tx-genset-sizing"},{key:"bz",page:"page-bz",list:"bzList",addBtn:"bzAddBtn",name:"busbar-enclosure-sizing"}];function showToast(msg){var t=document.getElementById("appToast");t||(t=document.createElement("div"),t.id="appToast",t.setAttribute("role","status"),t.setAttribute("aria-live","polite"),document.body.appendChild(t)),t.textContent=msg,t.className="show",clearTimeout(showToast._h),showToast._h=setTimeout(function(){t.className=""},2600)}function fallbackCopy(text){var ta=document.createElement("textarea");ta.value=text,ta.style.position="fixed",ta.style.opacity="0",document.body.appendChild(ta),ta.focus(),ta.select();try{document.execCommand("copy")}catch{}document.body.removeChild(ta)}function copyToClipboard(text){navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(text).catch(function(){fallbackCopy(text)}):fallbackCopy(text)}function b64Encode(str){return btoa(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g,function(m,p1){return String.fromCharCode("0x"+p1)}))}function b64Decode(str){return decodeURIComponent(Array.prototype.map.call(atob(str),function(c){return"%"+("00"+c.charCodeAt(0).toString(16)).slice(-2)}).join(""))}function getFieldValue(el){return el.type==="checkbox"?el.checked:el.type==="radio"?el.checked?el.value:void 0:el.value}function setFieldValue(el,val){el.type==="checkbox"?el.checked=!!val:el.type==="radio"?el.checked=el.value===val:el.value=val}function fireEvents(el){el.dispatchEvent(new Event("input",{bubbles:!0})),el.dispatchEvent(new Event("change",{bubbles:!0}))}function serializeStatic(pageEl){var out={};return pageEl.querySelectorAll("input[id], select[id]").forEach(function(el){if(!el.closest(".db-card")){var v=getFieldValue(el);v!==void 0&&(out["id:"+el.id]=v)}}),pageEl.querySelectorAll("input[type=radio][name]:not([id])").forEach(function(el){el.closest(".db-card")||el.checked&&(out["name:"+el.name]=el.value)}),out}function applyStatic(pageEl,data){Object.keys(data||{}).forEach(function(k){var val=data[k];if(k.indexOf("id:")===0){var el=document.getElementById(k.slice(3));el&&(setFieldValue(el,val),fireEvents(el))}else if(k.indexOf("name:")===0){var name=k.slice(5),els=pageEl.querySelectorAll('input[name="'+name+'"]'),last=null;els.forEach(function(el2){el2.checked=el2.value===val,last=el2}),last&&fireEvents(last)}})}function serializeCard(card){var out={};return card.querySelectorAll("input, select").forEach(function(el){var cls=Array.prototype.find.call(el.classList,function(c){return c!=="removeBtn"});if(cls){var v=getFieldValue(el);v!==void 0&&(out[cls]=v)}}),out.__id=card.dataset.id,out}function applyCard(card,data,idMap){Object.keys(data||{}).forEach(function(cls){if(cls!=="__id"){var val=data[cls];cls==="dbParent"&&idMap&&(val=val==="main"?"main":idMap[val]||val),card.querySelectorAll("."+cls).forEach(function(el){setFieldValue(el,val)})}}),card.querySelectorAll("input, select").forEach(fireEvents)}function buildPayload(cfg){var pageEl=document.getElementById(cfg.page),listEl=document.getElementById(cfg.list),cards=listEl?Array.prototype.slice.call(listEl.querySelectorAll(":scope > .db-card")):[];return{tool:cfg.name,savedAt:new Date().toISOString(),static:serializeStatic(pageEl),cards:cards.map(serializeCard)}}function applyPayload(cfg,payload){var pageEl=document.getElementById(cfg.page),listEl=document.getElementById(cfg.list),addBtn=document.getElementById(cfg.addBtn);if(!(!pageEl||!listEl||!addBtn)){Array.prototype.slice.call(listEl.querySelectorAll(":scope > .db-card")).forEach(function(c){var rm=c.querySelector(".removeBtn");rm?rm.click():c.remove()});for(var wantedCards=payload.cards||[],i=0;i<wantedCards.length;i++)addBtn.click();applyStatic(pageEl,payload.static||{});var newCards=Array.prototype.slice.call(listEl.querySelectorAll(":scope > .db-card")),idMap={};newCards.forEach(function(card,idx){var oldId=wantedCards[idx]&&wantedCards[idx].__id;oldId&&(idMap[oldId]=card.dataset.id)}),newCards.forEach(function(card,idx){applyCard(card,wantedCards[idx]||{},idMap)}),showToast("Loaded "+cfg.name.replace("-"," ")+" \u2014 "+wantedCards.length+" card(s) restored.")}}function downloadJSON(obj,filename){var blob=new Blob([JSON.stringify(obj,null,2)],{type:"application/json"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url,a.download=filename,document.body.appendChild(a),a.click(),a.remove(),setTimeout(function(){URL.revokeObjectURL(url)},2e3)}function exportTool(cfg){downloadJSON(buildPayload(cfg),cfg.name+"-project.json"),showToast("Downloaded "+cfg.name+"-project.json")}function importTool(cfg,file){var reader=new FileReader;reader.onload=function(){try{var payload=JSON.parse(reader.result);applyPayload(cfg,payload)}catch{showToast("Could not read that file \u2014 is it a project file exported from this sheet?")}},reader.readAsText(file)}function copyShareLink(cfg){var payload=buildPayload(cfg),encoded=b64Encode(JSON.stringify(payload)),url=location.origin+location.pathname+"?d="+encoded+"#"+cfg.key;copyToClipboard(url),showToast("Share link copied \u2014 opening it loads this sheet with these exact numbers.")}document.querySelectorAll(".sheet-actions").forEach(function(bar){var key=bar.dataset.tool,cfg=TOOLS.filter(function(t){return t.key===key})[0];if(cfg){var fileInput=bar.querySelector(".importFileInput");bar.querySelector(".actExport").addEventListener("click",function(){exportTool(cfg)}),bar.querySelector(".actImport").addEventListener("click",function(){fileInput.click()}),fileInput.addEventListener("change",function(){fileInput.files[0]&&importTool(cfg,fileInput.files[0]),fileInput.value=""}),bar.querySelector(".actPrint").addEventListener("click",function(){window.print()}),bar.querySelector(".actShare").addEventListener("click",function(){copyShareLink(cfg)})}}),(function(){var params=new URLSearchParams(location.search),d=params.get("d");if(d)try{var payload=JSON.parse(b64Decode(d)),cfg=TOOLS.filter(function(t){return t.name===payload.tool})[0];cfg&&setTimeout(function(){applyPayload(cfg,payload)},0)}catch{}})(),document.addEventListener("click",function(e){var el=e.target.closest(".sval, .rval, .tree-ik, #mainIk");if(el){var text=el.textContent.trim(),num=(text.match(/-?[\d.]+/)||[""])[0];num&&(copyToClipboard(num),showToast("Copied "+num+" \u2014 paste it into a field on another sheet."))}});var themeBtn=document.getElementById("themeToggleBtn");themeBtn&&themeBtn.addEventListener("click",function(){var isDark=document.documentElement.getAttribute("data-theme")==="dark";document.documentElement.setAttribute("data-theme",isDark?"light":"dark"),themeBtn.textContent=isDark?"\u263E Dark mode":"\u2600 Light mode"});var titleEl=document.querySelector(".masthead h1"),subEl=document.querySelector(".masthead-sub"),logoImg=document.getElementById("mastheadLogo"),editBtn=document.getElementById("editHeaderBtn"),logoInput=document.getElementById("logoFileInput"),saveHeaderBtn=document.getElementById("saveHeaderBtn"),loadHeaderBtn=document.getElementById("loadHeaderBtn"),headerFileInput=document.getElementById("headerFileInput");editBtn&&editBtn.addEventListener("click",function(){var editing=titleEl.getAttribute("contenteditable")==="true";editing?(titleEl.removeAttribute("contenteditable"),subEl.removeAttribute("contenteditable"),editBtn.textContent="\u270E Edit header",logoInput.style.display="none"):(titleEl.setAttribute("contenteditable","true"),subEl.setAttribute("contenteditable","true"),editBtn.textContent="\u2713 Done editing",logoInput.style.display="",showToast("Click the title or subtitle to rename them, or choose a logo file."))}),logoInput&&logoInput.addEventListener("change",function(){var f=logoInput.files[0];if(f){var reader=new FileReader;reader.onload=function(){logoImg.src=reader.result,logoImg.style.display=""},reader.readAsDataURL(f)}}),saveHeaderBtn&&saveHeaderBtn.addEventListener("click",function(){downloadJSON({title:titleEl.textContent,sub:subEl.textContent,logo:logoImg.src&&logoImg.style.display!=="none"?logoImg.src:null},"site-header.json"),showToast("Downloaded site-header.json")}),loadHeaderBtn&&(loadHeaderBtn.addEventListener("click",function(){headerFileInput.click()}),headerFileInput.addEventListener("change",function(){var f=headerFileInput.files[0];if(f){var reader=new FileReader;reader.onload=function(){try{var data=JSON.parse(reader.result);data.title&&(titleEl.textContent=data.title),data.sub&&(subEl.textContent=data.sub),data.logo&&(logoImg.src=data.logo,logoImg.style.display=""),showToast("Header loaded.")}catch{showToast("Could not read that header file.")}},reader.readAsText(f),headerFileInput.value=""}}));function mm2ToAwg(a){var d=Math.sqrt(4*a/Math.PI);return 36-39*Math.log(d/.127)/Math.log(92)}function attachUnitHints(root){(root||document).querySelectorAll("input.dbCsa, input.csS").forEach(function(el){if(el.dataset.hintOn)return;el.dataset.hintOn="1";var field=el.closest(".field");if(!field)return;var hint=document.createElement("div");hint.className="unit-hint",field.appendChild(hint);function upd(){var v=parseFloat(el.value);hint.textContent=isFinite(v)&&v>0?"\u2248 AWG "+mm2ToAwg(v).toFixed(1):""}el.addEventListener("input",upd),upd()}),(root||document).querySelectorAll("input.dbLen").forEach(function(el){if(el.dataset.hintOn)return;el.dataset.hintOn="1";var field=el.closest(".field");if(!field)return;var hint=document.createElement("div");hint.className="unit-hint",field.appendChild(hint);function upd(){var v=parseFloat(el.value);hint.textContent=isFinite(v)&&v>0?"\u2248 "+(v*3.28084).toFixed(1)+" ft":""}el.addEventListener("input",upd),upd()})}var EMPTY_IDS={dbList:"dbListEmpty",vdList:"vdListEmpty",selList:"selListEmpty",csList:"csListEmpty",estOutList:"estOutListEmpty"};function updateEmptyState(listId){var listEl=document.getElementById(listId),emptyEl=document.getElementById(EMPTY_IDS[listId]);!listEl||!emptyEl||(emptyEl.style.display=listEl.children.length?"none":"flex")}function updateAllEmptyStates(){Object.keys(EMPTY_IDS).forEach(updateEmptyState)}updateAllEmptyStates(),attachUnitHints(document);var hintObserver=new MutationObserver(function(muts){muts.forEach(function(m){m.addedNodes&&m.addedNodes.forEach(function(n){n.nodeType===1&&attachUnitHints(n.matches&&n.matches(".db-card")?n.parentNode:n)}),m.target&&m.target.id&&EMPTY_IDS[m.target.id]&&updateEmptyState(m.target.id)})});TOOLS.forEach(function(t){var listEl=document.getElementById(t.list);listEl&&hintObserver.observe(listEl,{childList:!0})})})()}catch(e){console.error("app.js module #17 error:",e)}try{(function(){function num(id){var el2=document.getElementById(id);return el2?parseFloat(el2.value):NaN}function el(id){return document.getElementById(id)}function fmt(v,d){return isFinite(v)?v.toFixed(d===void 0?2:d):"\u2014"}function setText(id,html){var e=el(id);e&&(e.innerHTML=html)}function calcPipeFlow(){if(el("pfQ")){var Q=num("pfQ"),D=num("pfD"),L=num("pfL"),eps=parseFloat(el("pfMat").value),dz=num("pfDz"),K=num("pfK"),nu=num("pfNu")*1e-6,rho=num("pfRho"),errs=0;if(errs+=fieldError(el("pfQ"),isNaN(Q)||Q<=0?"Flow rate must be greater than 0.":null),errs+=fieldError(el("pfD"),isNaN(D)||D<=0?"Diameter must be greater than 0.":null),errs+=fieldError(el("pfL"),isNaN(L)||L<=0?"Length must be greater than 0.":null),errs>0){updateValidationBanner("pfValidation",errs);return}var Dm=D/1e3,Qm3s=Q/1e3,A=Math.PI*Dm*Dm/4,v=Qm3s/A,Re=v*Dm/nu,relRough=eps/1e3/Dm,f,regime;Re<2300?(f=64/Re,regime="Laminar"):Re<4e3?(f=.25/Math.pow(Math.log(relRough/3.7+5.74/Math.pow(Re,.9))/Math.LN10,2),regime="Transitional"):(f=.25/Math.pow(Math.log(relRough/3.7+5.74/Math.pow(Re,.9))/Math.LN10,2),regime="Turbulent");var hf=f*(L/Dm)*(v*v)/(2*9.81),hMinor=K*(v*v)/(2*9.81),hFriction=hf+hMinor,hTotal=hFriction+dz,dpPa=rho*9.81*hFriction;setText("pfHf",fmt(hFriction,3)+" <small>m</small>"),setText("pfV",fmt(v,2)+" m/s"),setText("pfRe",Math.round(Re).toLocaleString()),setText("pfRegime",regime),setText("pfF",fmt(f,4)),setText("pfHtotal",fmt(hTotal,3)+" m"),setText("pfDp",fmt(dpPa/1e3,2)+" kPa <small>("+fmt(dpPa/1e5,3)+" bar)</small>");var banner=el("pfValidation");v>3?(banner.className="validation-banner bad",banner.innerHTML='<span class="vdot"></span><span>Velocity is '+fmt(v,2)+" m/s \u2014 above the usual 2\u20133 m/s guideline for water pipes (erosion / noise risk). Consider a larger diameter.</span>"):updateValidationBanner("pfValidation",0)}}["pfQ","pfD","pfL","pfMat","pfDz","pfK","pfNu","pfRho"].forEach(function(id){var e=el(id);e&&e.addEventListener("input",calcPipeFlow),e&&e.addEventListener("change",calcPipeFlow)}),calcPipeFlow();var MOTOR_STD_KW=[.18,.25,.37,.55,.75,1.1,1.5,2.2,3,4,5.5,7.5,11,15,18.5,22,30,37,45,55,75,90,110,132,160,200,250,315];function calcPump(){if(el("ppQ")){var Q=num("ppQ"),H=num("ppH"),eta=num("ppEta"),etaM=num("ppEtaM"),rho=num("ppRho"),npshA=num("ppNpshA"),npshR=num("ppNpshR"),errs=0;if(errs+=fieldError(el("ppQ"),isNaN(Q)||Q<=0?"Flow rate must be greater than 0.":null),errs+=fieldError(el("ppH"),isNaN(H)||H<=0?"Head must be greater than 0.":null),errs+=fieldError(el("ppEta"),isNaN(eta)||eta<=0||eta>100?"Efficiency must be between 0 and 100%.":null),errs+=fieldError(el("ppEtaM"),isNaN(etaM)||etaM<=0||etaM>100?"Efficiency must be between 0 and 100%.":null),errs>0){updateValidationBanner("ppValidation",errs);return}var Qm3s=Q/1e3,Ph=rho*9.81*Qm3s*H/1e3,Ps=Ph/(eta/100),Pm=Ps/(etaM/100),stdMotor=MOTOR_STD_KW.find(function(k){return k>=Pm})||MOTOR_STD_KW[MOTOR_STD_KW.length-1];setText("ppMotorStd",fmt(stdMotor,2)+" <small>kW</small>"),setText("ppPh",fmt(Ph,2)+" kW"),setText("ppPs",fmt(Ps,2)+" kW"),setText("ppPm",fmt(Pm,2)+" kW");var margin=npshA-npshR;setText("ppNpshMargin",fmt(margin,2)+" m");var st=el("ppNpshStatus");isNaN(margin)?(st.className="status-banner neutral",st.innerHTML='<span class="status-dot"></span><span>Enter NPSH values above.</span>'):margin<.5?(st.className="status-banner bad",st.innerHTML='<span class="status-dot"></span><span>Margin only '+fmt(margin,2)+" m \u2014 cavitation risk. Raise NPSH available (shorter/wider suction line, higher supply level) or pick a pump with lower NPSHr.</span>"):margin<1?(st.className="status-banner neutral",st.innerHTML='<span class="status-dot"></span><span>Margin '+fmt(margin,2)+" m \u2014 workable but tight. 1.0 m+ is a more comfortable margin.</span>"):(st.className="status-banner ok",st.innerHTML='<span class="status-dot"></span><span>Margin '+fmt(margin,2)+" m \u2014 comfortable NPSH margin.</span>"),updateValidationBanner("ppValidation",0)}}["ppQ","ppH","ppEta","ppEtaM","ppRho","ppNpshA","ppNpshR"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcPump),e.addEventListener("change",calcPump))}),calcPump();function calcHvac(){if(el("hvVel")){var mode=(document.querySelector('input[name="hvMode"]:checked')||{}).value||"ach";el("hvFldAch").style.display=mode==="ach"?"":"none",el("hvFldDirect").style.display=mode==="direct"?"":"none";var vel=num("hvVel"),rectH=num("hvRectH"),errs=0,Qls;if(mode==="ach"){var L=num("hvL"),W=num("hvW"),H=num("hvH"),ach=num("hvACH");errs+=fieldError(el("hvL"),isNaN(L)||L<=0?"Must be greater than 0.":null),errs+=fieldError(el("hvW"),isNaN(W)||W<=0?"Must be greater than 0.":null),errs+=fieldError(el("hvH"),isNaN(H)||H<=0?"Must be greater than 0.":null),errs+=fieldError(el("hvACH"),isNaN(ach)||ach<=0?"Must be greater than 0.":null);var vol=L*W*H;Qls=vol*ach*1e3/3600}else{var Qd=num("hvQdirect");errs+=fieldError(el("hvQdirect"),isNaN(Qd)||Qd<=0?"Must be greater than 0.":null),Qls=Qd}if(errs+=fieldError(el("hvVel"),isNaN(vel)||vel<=0?"Velocity must be greater than 0.":null),errs+=fieldError(el("hvRectH"),isNaN(rectH)||rectH<=0?"Height must be greater than 0.":null),errs>0){updateValidationBanner("hvValidation",errs);return}var Qm3s=Qls/1e3,Qm3h=Qls*3.6,Qcfm=Qls*2.11888,area=Qm3s/vel,Dcirc=Math.sqrt(4*area/Math.PI)*1e3,rectW=area*1e6/rectH;setText("hvDcirc",Math.round(Dcirc)+" <small>mm</small>"),setText("hvQLs",fmt(Qls,0)+" L/s"),setText("hvQm3h",fmt(Qm3h,0)+" m\xB3/h"),setText("hvQcfm",fmt(Qcfm,0)+" CFM"),setText("hvArea",fmt(area,3)+" m\xB2"),setText("hvRect",Math.round(rectH)+" \xD7 "+Math.round(rectW)+" mm"),updateValidationBanner("hvValidation",0)}}["hvL","hvW","hvH","hvACH","hvQdirect","hvVel","hvRectH"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcHvac),e.addEventListener("change",calcHvac))}),document.querySelectorAll('input[name="hvMode"]').forEach(function(r){r.addEventListener("change",calcHvac)}),calcHvac();function calcHvacDuct(){if(el("hdVel")){var mode=(document.querySelector('input[name="hdMode"]:checked')||{}).value||"ach";el("hdFldAch").style.display=mode==="ach"?"":"none",el("hdFldDirect").style.display=mode==="direct"?"":"none";var vel=num("hdVel"),rectH=num("hdRectH"),errs=0,Qls;if(mode==="ach"){var L=num("hdL"),W=num("hdW"),H=num("hdH"),ach=num("hdACH");errs+=fieldError(el("hdL"),isNaN(L)||L<=0?"Must be greater than 0.":null),errs+=fieldError(el("hdW"),isNaN(W)||W<=0?"Must be greater than 0.":null),errs+=fieldError(el("hdH"),isNaN(H)||H<=0?"Must be greater than 0.":null),errs+=fieldError(el("hdACH"),isNaN(ach)||ach<=0?"Must be greater than 0.":null);var vol=L*W*H;Qls=vol*ach*1e3/3600}else{var Qd=num("hdQdirect");errs+=fieldError(el("hdQdirect"),isNaN(Qd)||Qd<=0?"Must be greater than 0.":null),Qls=Qd}if(errs+=fieldError(el("hdVel"),isNaN(vel)||vel<=0?"Velocity must be greater than 0.":null),errs+=fieldError(el("hdRectH"),isNaN(rectH)||rectH<=0?"Height must be greater than 0.":null),errs>0){updateValidationBanner("hdValidation",errs);return}var Qm3s=Qls/1e3,Qm3h=Qls*3.6,Qcfm=Qls*2.11888,area=Qm3s/vel,Dcirc=Math.sqrt(4*area/Math.PI)*1e3,rectW=area*1e6/rectH;setText("hdDcirc",Math.round(Dcirc)+" <small>mm</small>"),setText("hdQLs",fmt(Qls,0)+" L/s"),setText("hdQm3h",fmt(Qm3h,0)+" m\xB3/h"),setText("hdQcfm",fmt(Qcfm,0)+" CFM"),setText("hdArea",fmt(area,3)+" m\xB2"),setText("hdRect",Math.round(rectH)+" \xD7 "+Math.round(rectW)+" mm"),updateValidationBanner("hdValidation",0)}}["hdL","hdW","hdH","hdACH","hdQdirect","hdVel","hdRectH"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcHvacDuct),e.addEventListener("change",calcHvacDuct))}),document.querySelectorAll('input[name="hdMode"]').forEach(function(r){r.addEventListener("change",calcHvacDuct)}),calcHvacDuct();function calcCoolingLoad(){if(el("clQls")){var Qls=num("clQls"),dt=num("clDt"),dw=num("clDw"),rho=num("clRho"),errs=0;if(errs+=fieldError(el("clQls"),isNaN(Qls)||Qls<=0?"Airflow must be greater than 0.":null),errs+=fieldError(el("clDt"),isNaN(dt)||dt<0?"Temperature drop can't be negative.":null),errs+=fieldError(el("clDw"),isNaN(dw)||dw<0?"Humidity ratio drop can't be negative.":null),errs+=fieldError(el("clRho"),isNaN(rho)||rho<=0?"Air density must be greater than 0.":null),errs>0){updateValidationBanner("clValidation",errs);return}var Qm3s=Qls/1e3,qSensible=1.005*rho*Qm3s*dt,qLatent=2501*rho*Qm3s*dw/1e3,qTotal=qSensible+qLatent,shr=qTotal>0?qSensible/qTotal:0,tons=qTotal/3.517;setText("clTotal",fmt(qTotal,2)+" <small>kW</small>"),setText("clSensible",fmt(qSensible,2)+" kW"),setText("clLatent",fmt(qLatent,2)+" kW"),setText("clShr",fmt(shr,2)),setText("clTons",fmt(tons,2)+" TR"),updateValidationBanner("clValidation",0)}}["clQls","clDt","clDw","clRho"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcCoolingLoad),e.addEventListener("change",calcCoolingLoad))}),calcCoolingLoad();function calcChiller(){if(el("chQcool")){var qCool=num("chQcool"),cop=num("chCop"),hLatent=num("chLatentHeat"),errs=0;if(errs+=fieldError(el("chQcool"),isNaN(qCool)||qCool<=0?"Cooling duty must be greater than 0.":null),errs+=fieldError(el("chCop"),isNaN(cop)||cop<=0?"COP must be greater than 0.":null),errs+=fieldError(el("chLatentHeat"),isNaN(hLatent)||hLatent<=0?"Latent heat must be greater than 0.":null),errs>0){updateValidationBanner("chValidation",errs);return}var pCompressor=qCool/cop,qReject=qCool+pCompressor,tons=qCool/3.517,mflow=qCool/hLatent*3600;setText("chPower",fmt(pCompressor,2)+" <small>kW</small>"),setText("chReject",fmt(qReject,2)+" kW"),setText("chTons",fmt(tons,2)+" TR"),setText("chMflow",fmt(mflow,1)+" kg/h"),updateValidationBanner("chValidation",0)}}["chQcool","chCop","chLatentHeat"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcChiller),e.addEventListener("change",calcChiller))}),calcChiller();function calcHeat(){if(el("thWm2")){var mode=(document.querySelector('input[name="thMode"]:checked')||{}).value||"airflow";el("thFldAirflow").style.display=mode==="airflow"?"":"none",el("thFldQuick").style.display=mode==="quick"?"":"none";var errs=0,kw;if(mode==="airflow"){var Q=num("thQ"),dt=num("thDt");errs+=fieldError(el("thQ"),isNaN(Q)||Q<=0?"Airflow must be greater than 0.":null),errs+=fieldError(el("thDt"),isNaN(dt)||dt<=0?"\u0394T must be greater than 0.":null),kw=1.2*Q*dt/1e3}else{var area=num("thArea"),wm2=num("thWm2");errs+=fieldError(el("thArea"),isNaN(area)||area<=0?"Area must be greater than 0.":null),errs+=fieldError(el("thWm2"),isNaN(wm2)||wm2<=0?"Load intensity must be greater than 0.":null),kw=area*wm2/1e3}if(errs>0){updateValidationBanner("thValidation",errs);return}setText("thKw",fmt(kw,2)+" <small>kW</small>"),setText("thBtu",Math.round(kw*3412.14).toLocaleString()+" BTU/h"),setText("thTons",fmt(kw*.28434,2)+" TR"),updateValidationBanner("thValidation",0)}}["thQ","thDt","thArea","thWm2"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcHeat),e.addEventListener("change",calcHeat))}),document.querySelectorAll('input[name="thMode"]').forEach(function(r){r.addEventListener("change",calcHeat)}),calcHeat();function calcBelt(){if(el("blD1")){var D1=num("blD1"),N1=num("blN1"),D2=num("blD2"),P=num("blP"),beltMax=parseFloat(el("blType").value),errs=0;if(errs+=fieldError(el("blD1"),isNaN(D1)||D1<=0?"Must be greater than 0.":null),errs+=fieldError(el("blN1"),isNaN(N1)||N1<=0?"Must be greater than 0.":null),errs+=fieldError(el("blD2"),isNaN(D2)||D2<=0?"Must be greater than 0.":null),errs+=fieldError(el("blP"),isNaN(P)||P<=0?"Must be greater than 0.":null),errs>0){updateValidationBanner("blValidation",errs);return}var N2=N1*D1/D2,ratio=D2/D1,vBelt=Math.PI*(D1/1e3)*N1/60,T1=9550*P/N1,T2=9550*P/N2;if(setText("blN2",fmt(N2,0)+" <small>RPM</small>"),setText("blRatio",fmt(ratio,2)+" : 1"),setText("blVbelt",fmt(vBelt,2)+" m/s"),setText("blT1",fmt(T1,1)+" Nm"),setText("blT2",fmt(T2,1)+" Nm"),vBelt>beltMax){var b=el("blValidation");b.className="validation-banner bad",b.innerHTML='<span class="vdot"></span><span>Belt speed '+fmt(vBelt,1)+" m/s exceeds the typical limit ("+beltMax+" m/s) for the selected belt type \u2014 reconsider the pulley sizes or belt type.</span>"}else updateValidationBanner("blValidation",0)}}["blD1","blN1","blD2","blP","blType"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcBelt),e.addEventListener("change",calcBelt))}),calcBelt();function asRawRange(){var sel=el("asSignal").value;if(sel==="__custom__")return[num("asRawLo"),num("asRawHi")];var parts=sel.split(",");return[parseFloat(parts[0]),parseFloat(parts[1])]}function calcSignalScale(){if(el("asSignal")){el("asCustomWrap").style.display=el("asSignal").value==="__custom__"?"":"none";var range=asRawRange(),rawLo=range[0],rawHi=range[1],euLo=num("asEuLo"),euHi=num("asEuHi"),unit=(el("asEuUnit").value||"").trim(),rawIn=num("asRawIn"),euIn=num("asEuIn"),errs=0;if(errs+=fieldError(el("asEuLo"),isNaN(euLo)?"Required.":null),errs+=fieldError(el("asEuHi"),isNaN(euHi)||euHi===euLo?"Must differ from the low value.":null),el("asSignal").value==="__custom__"&&(errs+=fieldError(el("asRawHi"),isNaN(rawLo)||isNaN(rawHi)||rawHi===rawLo?"Raw range must be two different numbers.":null)),errs+=fieldError(el("asRawIn"),isNaN(rawIn)?"Required.":null),errs+=fieldError(el("asEuIn"),isNaN(euIn)?"Required.":null),errs>0){updateValidationBanner("asValidation",errs);return}var euOut=euLo+(rawIn-rawLo)/(rawHi-rawLo)*(euHi-euLo),rawOut=rawLo+(euIn-euLo)/(euHi-euLo)*(rawHi-rawLo);setText("asEuOut",fmt(euOut,2)+(unit?" "+unit:"")),setText("asRawOut",fmt(rawOut,3));var banner=el("asValidation");rawIn<Math.min(rawLo,rawHi)-1e-9||rawIn>Math.max(rawLo,rawHi)+1e-9?(banner.className="validation-banner bad",banner.innerHTML='<span class="vdot"></span><span>Raw reading is outside the configured '+rawLo+"\u2013"+rawHi+" range \u2014 check for a sensor fault or an out-of-range process value.</span>"):updateValidationBanner("asValidation",0)}}["asSignal","asRawLo","asRawHi","asEuLo","asEuHi","asEuUnit","asRawIn","asEuIn"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcSignalScale),e.addEventListener("change",calcSignalScale))}),calcSignalScale();function calcPidTuning(){if(el("pidK")){var K=num("pidK"),tau=num("pidTau"),L=num("pidL"),type=(document.querySelector('input[name="pidType"]:checked')||{}).value||"PID",errs=0;if(errs+=fieldError(el("pidK"),isNaN(K)||K===0?"Process gain must not be 0.":null),errs+=fieldError(el("pidTau"),isNaN(tau)||tau<=0?"Time constant must be greater than 0.":null),errs+=fieldError(el("pidL"),isNaN(L)||L<=0?"Dead time must be greater than 0.":null),errs>0){updateValidationBanner("pidValidation",errs);return}var Kp,Ti,Td;type==="P"?(Kp=tau/(K*L),Ti=NaN,Td=NaN):type==="PI"?(Kp=.9*tau/(K*L),Ti=L/.3,Td=NaN):(Kp=1.2*tau/(K*L),Ti=2*L,Td=.5*L),Kp=Math.abs(Kp),setText("pidKp",fmt(Kp,3)),setText("pidTi",isNaN(Ti)?"\u2014 (not used)":fmt(Ti,2)+" s"),setText("pidTd",isNaN(Td)?"\u2014 (not used)":fmt(Td,2)+" s"),setText("pidKi",isNaN(Ti)?"\u2014":fmt(Kp/Ti,4)+" /s"),setText("pidKd",isNaN(Td)?"\u2014":fmt(Kp*Td,3)+" s"),updateValidationBanner("pidValidation",0)}}["pidK","pidTau","pidL"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcPidTuning),e.addEventListener("change",calcPidTuning))}),document.querySelectorAll('input[name="pidType"]').forEach(function(r){r.addEventListener("change",calcPidTuning)}),calcPidTuning();function calcValveSizing(){if(el("cvQ")){var Q=num("cvQ"),dP=num("cvDp"),sg=num("cvSg"),errs=0;if(errs+=fieldError(el("cvQ"),isNaN(Q)||Q<=0?"Flow rate must be greater than 0.":null),errs+=fieldError(el("cvDp"),isNaN(dP)||dP<=0?"Pressure drop must be greater than 0.":null),errs+=fieldError(el("cvSg"),isNaN(sg)||sg<=0?"Specific gravity must be greater than 0.":null),errs>0){updateValidationBanner("cvValidation",errs);return}var Kv=Q*Math.sqrt(sg/dP),Cv=Kv*1.156;setText("cvKv",fmt(Kv,2)),setText("cvCv",fmt(Cv,2)),updateValidationBanner("cvValidation",0)}}["cvQ","cvDp","cvSg"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcValveSizing),e.addEventListener("change",calcValveSizing))}),calcValveSizing();function calcLevelRanging(){if(el("lvRho")){var rho=num("lvRho"),H=num("lvH"),errs=0;if(errs+=fieldError(el("lvRho"),isNaN(rho)||rho<=0?"Density must be greater than 0.":null),errs+=fieldError(el("lvH"),isNaN(H)||H<=0?"Level span must be greater than 0.":null),errs>0){updateValidationBanner("lvValidation",errs);return}var pPa=rho*9.81*H,pKpa=pPa/1e3,pMbar=pKpa*10,pInh2o=pKpa*4.0147;setText("lvKpa",fmt(pKpa,2)+" <small>kPa</small>"),setText("lvMbar",fmt(pMbar,1)+" mbar"),setText("lvInh2o",fmt(pInh2o,2)+" inH\u2082O"),setText("lv20ma","100% level, "+fmt(pKpa,2)+" kPa"),updateValidationBanner("lvValidation",0)}}["lvRho","lvH"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcLevelRanging),e.addEventListener("change",calcLevelRanging))}),calcLevelRanging();function calcVfdScaling(){if(el("vfFrated")){var fRated=num("vfFrated"),nRated=num("vfNrated"),poles=num("vfPoles"),fTarget=num("vfFtarget"),errs=0;if(errs+=fieldError(el("vfFrated"),isNaN(fRated)||fRated<=0?"Must be greater than 0.":null),errs+=fieldError(el("vfNrated"),isNaN(nRated)||nRated<=0?"Must be greater than 0.":null),errs+=fieldError(el("vfPoles"),isNaN(poles)||poles<=0||poles%2!==0?"Must be a positive even number.":null),errs+=fieldError(el("vfFtarget"),isNaN(fTarget)||fTarget<=0?"Must be greater than 0.":null),errs>0){updateValidationBanner("vfValidation",errs);return}var nsRated=120*fRated/poles,slipRated=nsRated-nRated,nsTarget=120*fTarget/poles,nTarget=nsTarget-slipRated;setText("vfNout",Math.round(nTarget)+" <small>RPM</small>"),setText("vfNsRated",Math.round(nsRated)+" RPM"),setText("vfSlipRated",fmt(slipRated,1)+" RPM"),setText("vfNsTarget",Math.round(nsTarget)+" RPM"),setText("vfPct",fmt(fTarget/fRated*100,1)+"%");var banner=el("vfValidation");slipRated<0?(banner.className="validation-banner bad",banner.innerHTML='<span class="vdot"></span><span>Rated speed is above synchronous speed for this pole count \u2014 check the pole count and nameplate speed.</span>'):updateValidationBanner("vfValidation",0)}}["vfFrated","vfNrated","vfPoles","vfFtarget"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcVfdScaling),e.addEventListener("change",calcVfdScaling))}),calcVfdScaling()})()}catch(e){console.error("app.js module #19 error:",e)}try{var CIVIL_NOTES_HTML=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
`}catch(e){console.error("app.js module #20 error:",e)}try{(function(){function num(id){var el2=document.getElementById(id);return el2?parseFloat(el2.value):NaN}function el(id){return document.getElementById(id)}function fmt(v,d){return isFinite(v)?v.toFixed(d===void 0?2:d):"\u2014"}function setText(id,html){var e=el(id);e&&(e.innerHTML=html)}function calcBeam(){if(el("cvbSpan")){var L=num("cvbSpan"),type=el("cvbType").value,w=num("cvbW"),limitRatio=num("cvbLimit"),Z=num("cvbZ"),I=num("cvbI"),E=num("cvbE"),allow=num("cvbAllow"),errs=0;if(errs+=fieldError(el("cvbSpan"),isNaN(L)||L<=0?"Span must be greater than 0.":null),errs+=fieldError(el("cvbW"),isNaN(w)||w<=0?"Load must be greater than 0.":null),errs+=fieldError(el("cvbZ"),isNaN(Z)||Z<=0?"Section modulus must be greater than 0.":null),errs+=fieldError(el("cvbI"),isNaN(I)||I<=0?"Moment of inertia must be greater than 0.":null),errs>0){updateValidationBanner("cvBeamValidation",errs);return}var Lmm=L*1e3,Zmm3=Z*1e3,Imm4=I*1e4,Empa=E*1e3,M,defl;if(type==="udl"){M=w*L*L/8;var wNmm=w;defl=5*wNmm*Math.pow(Lmm,4)/(384*Empa*Imm4)}else{var Pn=w*1e3;M=w*L/4,defl=Pn*Math.pow(Lmm,3)/(48*Empa*Imm4)}var Mnmm=M*1e6,sigma=Mnmm/Zmm3,util=sigma/allow,deflLimitMm=Lmm/limitRatio;setText("cvbM",fmt(M,2)+" <small>kNm</small>"),setText("cvbSigma",fmt(sigma,1)+" MPa"),setText("cvbUtil",fmt(util*100,0)+"%"),setText("cvbDefl",fmt(defl,2)+" mm"),setText("cvbDeflLimit",fmt(deflLimitMm,1)+" mm <small>(L/"+limitRatio+")</small>");var bad=util>1||defl>deflLimitMm,banner=el("cvBeamValidation");if(bad){banner.className="validation-banner bad";var msgs=[];util>1&&msgs.push("bending stress exceeds allowable ("+fmt(util*100,0)+"%)"),defl>deflLimitMm&&msgs.push("deflection exceeds the L/"+limitRatio+" limit"),banner.innerHTML='<span class="vdot"></span><span>Fails: '+msgs.join(" and ")+".</span>"}else updateValidationBanner("cvBeamValidation",0)}}["cvbSpan","cvbType","cvbW","cvbLimit","cvbZ","cvbI","cvbE","cvbAllow"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcBeam),e.addEventListener("change",calcBeam))}),calcBeam();function calcColumn(){if(el("cvcLe")){var Le=num("cvcLe"),N=num("cvcN"),I=num("cvcI"),A=num("cvcA"),E=num("cvcE"),fos=num("cvcFos"),errs=0;if(errs+=fieldError(el("cvcLe"),isNaN(Le)||Le<=0?"Effective length must be greater than 0.":null),errs+=fieldError(el("cvcI"),isNaN(I)||I<=0?"Moment of inertia must be greater than 0.":null),errs+=fieldError(el("cvcA"),isNaN(A)||A<=0?"Area must be greater than 0.":null),errs>0){updateValidationBanner("cvColValidation",errs);return}var Lemm=Le*1e3,Imm4=I*1e4,Amm2=A*100,Empa=E*1e3,Pcr=Math.PI*Math.PI*Empa*Imm4/(Lemm*Lemm),PcrKn=Pcr/1e3,r=Math.sqrt(I/A)*10,rcm=Math.sqrt(I/A),slend=Le*1e3/(rcm*10),allowKn=PcrKn/fos,util=N/allowKn;setText("cvcAllow",fmt(allowKn,1)+" <small>kN</small>"),setText("cvcPcr",fmt(PcrKn,1)+" kN"),setText("cvcR",fmt(rcm*10,1)+" mm"),setText("cvcSlend",fmt(slend,0)),setText("cvcUtil",fmt(util*100,0)+"%");var banner=el("cvColValidation");util>1?(banner.className="validation-banner bad",banner.innerHTML='<span class="vdot"></span><span>Applied load exceeds allowable capacity ('+fmt(util*100,0)+"% utilized).</span>"):slend>200?(banner.className="validation-banner bad",banner.innerHTML='<span class="vdot"></span><span>Slenderness ratio '+fmt(slend,0)+" is impractically high (&gt;200) \u2014 reconsider the section or bracing.</span>"):updateValidationBanner("cvColValidation",0)}}["cvcLe","cvcN","cvcI","cvcA","cvcE","cvcFos"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcColumn),e.addEventListener("change",calcColumn))}),calcColumn();function calcFooting(){if(el("cvfN")){var N=num("cvfN"),qa=num("cvfQa"),swPct=num("cvfSw"),roundMm=num("cvfRound"),errs=0;if(errs+=fieldError(el("cvfN"),isNaN(N)||N<=0?"Load must be greater than 0.":null),errs+=fieldError(el("cvfQa"),isNaN(qa)||qa<=0?"Allowable bearing pressure must be greater than 0.":null),errs>0){updateValidationBanner("cvFtgValidation",errs);return}var totalLoad=N*(1+swPct/100),reqArea=totalLoad/qa,sideM=Math.sqrt(reqArea),roundM=(roundMm>0?roundMm:50)/1e3,sideRounded=Math.ceil(sideM/roundM)*roundM,actualArea=sideRounded*sideRounded,actualQ=totalLoad/actualArea,util=actualQ/qa;setText("cvfSize",fmt(sideRounded,2)+" <small>m \xD7 "+fmt(sideRounded,2)+" m</small>"),setText("cvfArea",fmt(reqArea,2)+" m\xB2"),setText("cvfTotalLoad",fmt(totalLoad,1)+" kN"),setText("cvfActualQ",fmt(actualQ,1)+" kPa"),setText("cvfUtil",fmt(util*100,0)+"%"),updateValidationBanner("cvFtgValidation",0)}}["cvfN","cvfQa","cvfSw","cvfRound"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcFooting),e.addEventListener("change",calcFooting))}),calcFooting();function calcWall(){if(el("cvwH")){var H=num("cvwH"),gamma=num("cvwGamma"),phiDeg=num("cvwPhi"),B=num("cvwB"),W=num("cvwW"),mu=num("cvwMu"),fosOtReq=num("cvwFosOt"),fosSlReq=num("cvwFosSl"),errs=0;if(errs+=fieldError(el("cvwH"),isNaN(H)||H<=0?"Height must be greater than 0.":null),errs+=fieldError(el("cvwB"),isNaN(B)||B<=0?"Base width must be greater than 0.":null),errs+=fieldError(el("cvwW"),isNaN(W)||W<=0?"Wall weight must be greater than 0.":null),errs>0){updateValidationBanner("cvWallValidation",errs);return}var phi=phiDeg*Math.PI/180,Ka=(1-Math.sin(phi))/(1+Math.sin(phi)),Pa=.5*Ka*gamma*H*H,Mo=Pa*(H/3),Mr=W*(B/2),fosOt=Mr/Mo,fosSl=mu*W/Pa;setText("cvwPa",fmt(Pa,2)+" <small>kN/m</small>"),setText("cvwKa",fmt(Ka,3)),setText("cvwFosOtResult",fmt(fosOt,2)+" <small>(need \u2265"+fmt(fosOtReq,1)+")</small>"),setText("cvwFosSlResult",fmt(fosSl,2)+" <small>(need \u2265"+fmt(fosSlReq,1)+")</small>");var otOk=fosOt>=fosOtReq,slOk=fosSl>=fosSlReq;setText("cvwStatus",otOk&&slOk?"PASS":"FAIL");var banner=el("cvWallValidation");if(!otOk||!slOk){banner.className="validation-banner bad";var msgs=[];otOk||msgs.push("overturning FOS "+fmt(fosOt,2)+" is below the required "+fmt(fosOtReq,1)),slOk||msgs.push("sliding FOS "+fmt(fosSl,2)+" is below the required "+fmt(fosSlReq,1)),banner.innerHTML='<span class="vdot"></span><span>Fails: '+msgs.join(" and ")+".</span>"}else updateValidationBanner("cvWallValidation",0)}}["cvwH","cvwGamma","cvwPhi","cvwB","cvwW","cvwMu","cvwFosOt","cvwFosSl"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcWall),e.addEventListener("change",calcWall))}),calcWall();var civFrame=document.getElementById("civNotesFrame");civFrame&&(civFrame.srcdoc=CIVIL_NOTES_HTML)})()}catch(e){console.error("app.js module #21 error:",e)}try{document.addEventListener("DOMContentLoaded",function(){var catSelect=document.getElementById("civNoteCatSelect"),newCatWrap=document.getElementById("civNoteNewCatWrap");if(!catSelect)return;catSelect.addEventListener("change",function(){newCatWrap.style.display=this.value==="__new__"?"":"none"});function jsEscape(s){return String(s).replace(/\\/g,"\\\\").replace(/"/g,'\\"')}document.getElementById("civNoteGenBtn").addEventListener("click",function(){var title=document.getElementById("civNoteTitle").value.trim(),summary=document.getElementById("civNoteSummary").value.trim(),bodyLines=document.getElementById("civNoteBody").value.split(`
`).map(function(l){return l.trim()}).filter(Boolean),cat=catSelect.value,newCat=document.getElementById("civNoteNewCat").value.trim(),out=document.getElementById("civNoteOutput");if(!title||!summary||bodyLines.length===0){out.value="Fill in title, summary, and at least one body paragraph first.";return}var noteObj=`        {
          title:"`+jsEscape(title)+`",
          summary:"`+jsEscape(summary)+`",
          body:[
`+bodyLines.map(function(l){return'            "'+jsEscape(l)+'"'}).join(`,
`)+`
        ...
        }`;if(cat==="__new__"&&newCat){var slug=newCat.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"");out.value=`// New subject block \u2014 add this as a new entry in the DATA array (top level, alongside "structural" etc.):
    {
...
`+noteObj+","}),document.getElementById("civNoteCopyBtn").addEventListener("click",function(){var out=document.getElementById("civNoteOutput"),statusEl=document.getElementById("civNoteCopyStatus");if(!out.value){statusEl.textContent="Generate the code first.";return}out.select(),navigator.clipboard.writeText(out.value).then(function(){statusEl.textContent="Copied \u2014 paste it into the DATA array in the Notes page source."}).catch(function(){statusEl.textContent="Couldn't auto-copy \u2014 the text is selected, use Ctrl/Cmd+C."})})})}catch(e){console.error("app.js module #22 error:",e)}try{(function(){function num(id){var el2=document.getElementById(id);return el2?parseFloat(el2.value):NaN}function el(id){return document.getElementById(id)}function fmt(v,d){return isFinite(v)?v.toFixed(d===void 0?2:d):"\u2014"}function setText(id,html){var e=el(id);e&&(e.innerHTML=html)}var CATS={length:{label:"Length",units:[["mm","Millimetre (mm)"],["cm","Centimetre (cm)"],["m","Metre (m)"],["km","Kilometre (km)"],["in","Inch (in)"],["ft","Foot (ft)"],["yd","Yard (yd)"],["mi","Mile (mi)"]],factors:{mm:.001,cm:.01,m:1,km:1e3,in:.0254,ft:.3048,yd:.9144,mi:1609.344}},mass:{label:"Mass",units:[["g","Gram (g)"],["kg","Kilogram (kg)"],["t","Tonne (t)"],["oz","Ounce (oz)"],["lb","Pound (lb)"],["ust","US ton (short)"],["lt","Long ton"]],factors:{g:.001,kg:1,t:1e3,oz:.0283495231,lb:.45359237,ust:907.18474,lt:1016.0469088}},area:{label:"Area",units:[["mm2","mm\xB2"],["cm2","cm\xB2"],["m2","m\xB2"],["ha","Hectare"],["km2","km\xB2"],["in2","in\xB2"],["ft2","ft\xB2"],["acre","Acre"],["mi2","mi\xB2"]],factors:{mm2:1e-6,cm2:1e-4,m2:1,ha:1e4,km2:1e6,in2:64516e-8,ft2:.09290304,acre:4046.8564224,mi2:2589988110336e-6}},volume:{label:"Volume",units:[["ml","Millilitre (mL)"],["l","Litre (L)"],["m3","m\xB3"],["in3","in\xB3"],["ft3","ft\xB3"],["usgal","US gallon"],["ukgal","UK (imperial) gallon"]],factors:{ml:.001,l:1,m3:1e3,in3:.0163871,ft3:28.316846592,usgal:3.785411784,ukgal:4.54609}},pressure:{label:"Pressure",units:[["pa","Pascal (Pa)"],["kpa","Kilopascal (kPa)"],["mpa","Megapascal (MPa)"],["bar","Bar"],["psi","psi"],["atm","Atmosphere (atm)"],["mmhg","mmHg"]],factors:{pa:1,kpa:1e3,mpa:1e6,bar:1e5,psi:6894.757293168,atm:101325,mmhg:133.322387415}},temperature:{label:"Temperature",units:[["c","Celsius (\xB0C)"],["f","Fahrenheit (\xB0F)"],["k","Kelvin (K)"]],special:!0},force:{label:"Force",units:[["n","Newton (N)"],["kn","Kilonewton (kN)"],["lbf","Pound-force (lbf)"],["kgf","Kilogram-force (kgf)"]],factors:{n:1,kn:1e3,lbf:4.4482216153,kgf:9.80665}},torque:{label:"Torque",units:[["nm","Newton-metre (N\xB7m)"],["knm","Kilonewton-metre (kN\xB7m)"],["lbfft","Pound-force foot (lbf\xB7ft)"],["lbfin","Pound-force inch (lbf\xB7in)"]],factors:{nm:1,knm:1e3,lbfft:1.3558179483,lbfin:.112984829}},energy:{label:"Energy",units:[["j","Joule (J)"],["kj","Kilojoule (kJ)"],["kwh","Kilowatt-hour (kWh)"],["btu","BTU"],["cal","Calorie (cal)"],["kcal","Kilocalorie (kcal)"]],factors:{j:1,kj:1e3,kwh:36e5,btu:1055.05585262,cal:4.184,kcal:4184}},power:{label:"Power",units:[["w","Watt (W)"],["kw","Kilowatt (kW)"],["mw","Megawatt (MW)"],["hp","Horsepower, mechanical (hp)"],["btuh","BTU/h"]],factors:{w:1,kw:1e3,mw:1e6,hp:745.6998715823,btuh:.29307107}},speed:{label:"Speed",units:[["ms","Metres/second (m/s)"],["kmh","Kilometres/hour (km/h)"],["mph","Miles/hour (mph)"],["fts","Feet/second (ft/s)"],["knot","Knot"]],factors:{ms:1,kmh:.277777778,mph:.44704,fts:.3048,knot:.514444444}}},CAT_ORDER=["length","mass","area","volume","pressure","temperature","force","torque","energy","power","speed"];function tempToC(v,unit){return unit==="c"?v:unit==="f"?(v-32)*5/9:v-273.15}function tempFromC(c,unit){return unit==="c"?c:unit==="f"?c*9/5+32:c+273.15}function populateUnitConverter(){var catSel=el("ucCat");catSel&&(catSel.innerHTML="",CAT_ORDER.forEach(function(key){var o=document.createElement("option");o.value=key,o.textContent=CATS[key].label,catSel.appendChild(o)}),catSel.addEventListener("change",function(){populateUnitSelects(),convertUnits()}),populateUnitSelects(),["ucFromVal","ucFromUnit","ucToUnit"].forEach(function(id){el(id).addEventListener("input",convertUnits),el(id).addEventListener("change",convertUnits)}),convertUnits())}function populateUnitSelects(){var cat=CATS[el("ucCat").value],fromSel=el("ucFromUnit"),toSel=el("ucToUnit");fromSel.innerHTML="",toSel.innerHTML="",cat.units.forEach(function(pair,i){var o1=document.createElement("option");o1.value=pair[0],o1.textContent=pair[1];var o2=document.createElement("option");o2.value=pair[0],o2.textContent=pair[1],fromSel.appendChild(o1),toSel.appendChild(o2)}),fromSel.selectedIndex=0,toSel.selectedIndex=cat.units.length>1?1:0}function convertUnits(){var catKey=el("ucCat").value,cat=CATS[catKey],v=num("ucFromVal"),fromU=el("ucFromUnit").value,toU=el("ucToUnit").value;if(isNaN(v)){setText("ucResult","\u2014");return}var result;if(cat.special)result=tempFromC(tempToC(v,fromU),toU);else{var base=v*cat.factors[fromU];result=base/cat.factors[toU]}var toLabel=cat.units.filter(function(u){return u[0]===toU})[0];setText("ucResult",fmt(result,Math.abs(result)<1?6:4)+" <small>"+(toLabel?toLabel[0]:toU)+"</small>")}populateUnitConverter();var LANGS=[["es","Spanish"],["fr","French"],["pt","Portuguese"],["de","German"],["pl","Polish"]],TERMS=[{en:"Stop",es:"Alto / Pare",fr:"Arr\xEAt / Stop",pt:"Pare",de:"Stopp / Halt",pl:"Stop"},{en:"Danger",es:"Peligro",fr:"Danger",pt:"Perigo",de:"Gefahr",pl:"Niebezpiecze\u0144stwo"},{en:"Warning",es:"Advertencia",fr:"Avertissement",pt:"Aviso",de:"Warnung",pl:"Ostrze\u017Cenie"},{en:"Caution",es:"Precauci\xF3n",fr:"Attention",pt:"Cuidado",de:"Vorsicht",pl:"Uwaga"},{en:"High voltage",es:"Alto voltaje",fr:"Haute tension",pt:"Alta tens\xE3o",de:"Hochspannung",pl:"Wysokie napi\u0119cie"},{en:"Hard hat",es:"Casco de seguridad",fr:"Casque de s\xE9curit\xE9",pt:"Capacete de seguran\xE7a",de:"Schutzhelm",pl:"Kask ochronny"},{en:"Safety glasses",es:"Gafas de seguridad",fr:"Lunettes de s\xE9curit\xE9",pt:"\xD3culos de seguran\xE7a",de:"Schutzbrille",pl:"Okulary ochronne"},{en:"Safety vest",es:"Chaleco de seguridad",fr:"Gilet de s\xE9curit\xE9",pt:"Colete de seguran\xE7a",de:"Warnweste",pl:"Kamizelka odblaskowa"},{en:"Gloves",es:"Guantes",fr:"Gants",pt:"Luvas",de:"Handschuhe",pl:"R\u0119kawice"},{en:"Ear protection",es:"Protecci\xF3n auditiva",fr:"Protection auditive",pt:"Prote\xE7\xE3o auricular",de:"Geh\xF6rschutz",pl:"Ochrona s\u0142uchu"},{en:"Harness",es:"Arn\xE9s",fr:"Harnais",pt:"Arn\xEAs",de:"Sicherheitsgurt",pl:"Uprz\u0105\u017C bezpiecze\u0144stwa"},{en:"Wet floor",es:"Piso mojado",fr:"Sol mouill\xE9",pt:"Piso molhado",de:"Nasser Boden",pl:"Mokra pod\u0142oga"},{en:"No entry",es:"No entrar",fr:"Entr\xE9e interdite",pt:"Entrada proibida",de:"Kein Zutritt",pl:"Wst\u0119p wzbroniony"},{en:"Authorized personnel only",es:"Solo personal autorizado",fr:"Personnel autoris\xE9 seulement",pt:"Somente pessoal autorizado",de:"Nur autorisiertes Personal",pl:"Tylko dla upowa\u017Cnionego personelu"},{en:"Emergency exit",es:"Salida de emergencia",fr:"Sortie de secours",pt:"Sa\xEDda de emerg\xEAncia",de:"Notausgang",pl:"Wyj\u015Bcie awaryjne"},{en:"Fire",es:"Incendio / Fuego",fr:"Feu / Incendie",pt:"Inc\xEAndio / Fogo",de:"Feuer",pl:"Po\u017Car / Ogie\u0144"},{en:"Fire extinguisher",es:"Extintor de incendios",fr:"Extincteur",pt:"Extintor de inc\xEAndio",de:"Feuerl\xF6scher",pl:"Ga\u015Bnica"},{en:"First aid",es:"Primeros auxilios",fr:"Premiers secours",pt:"Primeiros socorros",de:"Erste Hilfe",pl:"Pierwsza pomoc"},{en:"Emergency",es:"Emergencia",fr:"Urgence",pt:"Emerg\xEAncia",de:"Notfall",pl:"Nag\u0142y wypadek"},{en:"Injured",es:"Herido",fr:"Bless\xE9",pt:"Ferido",de:"Verletzt",pl:"Ranny"},{en:"Hospital",es:"Hospital",fr:"H\xF4pital",pt:"Hospital",de:"Krankenhaus",pl:"Szpital"},{en:"Lockout/Tagout",es:"Bloqueo y etiquetado",fr:"Verrouillage/\xE9tiquetage",pt:"Bloqueio e etiquetagem",de:"Verriegelung und Kennzeichnung",pl:"Blokada i oznakowanie"},{en:"Confined space",es:"Espacio confinado",fr:"Espace confin\xE9",pt:"Espa\xE7o confinado",de:"Enger Raum",pl:"Przestrze\u0144 zamkni\u0119ta"},{en:"Trench",es:"Zanja",fr:"Tranch\xE9e",pt:"Vala",de:"Graben",pl:"Wykop"},{en:"Scaffolding",es:"Andamio",fr:"\xC9chafaudage",pt:"Andaime",de:"Ger\xFCst",pl:"Rusztowanie"},{en:"Crane",es:"Gr\xFAa",fr:"Grue",pt:"Guindaste",de:"Kran",pl:"D\u017Awig"},{en:"Forklift",es:"Montacargas",fr:"Chariot \xE9l\xE9vateur",pt:"Empilhadeira",de:"Gabelstapler",pl:"W\xF3zek wid\u0142owy"},{en:"Excavation",es:"Excavaci\xF3n",fr:"Excavation",pt:"Escava\xE7\xE3o",de:"Aushub",pl:"Wykop"},{en:"Overhead hazard",es:"Peligro a\xE9reo",fr:"Danger a\xE9rien",pt:"Perigo a\xE9reo",de:"Gefahr von oben",pl:"Zagro\u017Cenie z g\xF3ry"},{en:"Keep clear",es:"Mant\xE9ngase alejado",fr:"Restez \xE0 l\u2019\xE9cart",pt:"Mantenha-se afastado",de:"Bereich freihalten",pl:"Zachowa\u0107 odleg\u0142o\u015B\u0107"},{en:"Do you understand?",es:"\xBFEntiende?",fr:"Comprenez-vous ?",pt:"Voc\xEA entende?",de:"Verstehen Sie?",pl:"Czy rozumiesz?"},{en:"Yes",es:"S\xED",fr:"Oui",pt:"Sim",de:"Ja",pl:"Tak"},{en:"No",es:"No",fr:"Non",pt:"N\xE3o",de:"Nein",pl:"Nie"}];function langLabel(code){var m=LANGS.filter(function(l){return l[0]===code})[0];return m?m[1]:code}function renderTranslator(){var listEl=el("pbList");if(listEl){var term=(el("pbSearch").value||"").trim().toLowerCase(),langChoice=el("pbLang").value,shown=TERMS.filter(function(t){return!term||t.en.toLowerCase().indexOf(term)!==-1?!0:LANGS.some(function(l){return(t[l[0]]||"").toLowerCase().indexOf(term)!==-1})});listEl.innerHTML="",shown.forEach(function(t){var row=document.createElement("div");row.style.cssText="padding:12px 0;border-bottom:1px solid var(--line);";var enLine=document.createElement("div");enLine.textContent=t.en,enLine.style.cssText="font-family:var(--font-body);font-weight:600;color:var(--ink);margin-bottom:6px;",row.appendChild(enLine);var langsToShow=langChoice==="all"?LANGS:LANGS.filter(function(l){return l[0]===langChoice});if(langChoice==="all"){var grid=document.createElement("div");grid.style.cssText="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:6px 16px;",langsToShow.forEach(function(l){var cell=document.createElement("div");cell.style.cssText="font-family:var(--font-mono);font-size:12px;color:var(--ink-dim);",cell.innerHTML='<span style="text-transform:uppercase;letter-spacing:.04em;font-size:9.5px;color:var(--ink-faint);">'+l[1]+"</span><br>"+t[l[0]],grid.appendChild(cell)}),row.appendChild(grid)}else{var big=document.createElement("div");big.style.cssText="font-family:var(--font-body);font-size:16px;color:var(--blue);",big.textContent=t[langChoice],row.appendChild(big)}listEl.appendChild(row)}),setText("pbCount",shown.length+" of "+TERMS.length+" terms shown"+(langChoice!=="all"?" \u2014 translated to "+langLabel(langChoice)+".":", all languages."))}}el("pbSearch")&&(el("pbSearch").addEventListener("input",renderTranslator),el("pbLang").addEventListener("change",renderTranslator),renderTranslator());function parseDateInput(id){var v=el(id).value;if(!v)return null;var d=new Date(v+"T00:00:00");return isNaN(d.getTime())?null:d}var WEEKDAY_NAMES=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],MONTH_NAMES=["January","February","March","April","May","June","July","August","September","October","November","December"];function formatDate(d){return WEEKDAY_NAMES[d.getDay()]+", "+MONTH_NAMES[d.getMonth()]+" "+d.getDate()+", "+d.getFullYear()}function calcDateAdd(){var start=parseDateInput("dcStartDate"),days=num("dcDays");if(!start||isNaN(days)){setText("dcResultDate","\u2014");return}var result=new Date(start.getTime());result.setDate(result.getDate()+Math.round(days)),setText("dcResultDate",formatDate(result))}function calcDateBetween(){var a=parseDateInput("dcDateA"),b=parseDateInput("dcDateB");if(!a||!b){setText("dcTotalDays","\u2014"),setText("dcWeeks","\u2014"),setText("dcWeekdays","\u2014");return}for(var start=a<b?a:b,end=a<b?b:a,totalDays=Math.round((end-start)/864e5),weeks=Math.floor(totalDays/7),remDays=totalDays%7,weekdayCount=0,d=new Date(start.getTime()),i=0;i<totalDays;i++){d.setDate(d.getDate()+1);var wd=d.getDay();wd!==0&&wd!==6&&weekdayCount++}setText("dcTotalDays",totalDays+" <small>days</small>"),setText("dcWeeks",weeks+" wk "+remDays+" d"),setText("dcWeekdays",weekdayCount+" days")}if(["dcStartDate","dcDays"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcDateAdd),e.addEventListener("change",calcDateAdd))}),["dcDateA","dcDateB"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcDateBetween),e.addEventListener("change",calcDateBetween))}),el("dcStartDate")){var today=new Date,iso=today.getFullYear()+"-"+String(today.getMonth()+1).padStart(2,"0")+"-"+String(today.getDate()).padStart(2,"0");el("dcStartDate").value=iso,el("dcDateA").value=iso;var later=new Date(today.getTime());later.setDate(later.getDate()+14),el("dcDateB").value=later.getFullYear()+"-"+String(later.getMonth()+1).padStart(2,"0")+"-"+String(later.getDate()).padStart(2,"0"),calcDateAdd(),calcDateBetween()}function calcMarkupMargin(){if(el("mmCost")){var cost=num("mmCost"),sell=num("mmSell");if(isNaN(cost)||isNaN(sell)||cost<=0){setText("mmMarkup","\u2014"),setText("mmMargin","\u2014"),setText("mmProfit","\u2014");return}var profit=sell-cost,markup=profit/cost*100,margin=sell!==0?profit/sell*100:NaN;setText("mmMarkup",fmt(markup,1)+"<small>%</small>"),setText("mmMargin",fmt(margin,1)+"%"),setText("mmProfit",fmt(profit,2))}}["mmCost","mmSell"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcMarkupMargin),e.addEventListener("change",calcMarkupMargin))}),calcMarkupMargin();function calcPctChange(){if(el("pcOld")){var o=num("pcOld"),n=num("pcNew");if(isNaN(o)||isNaN(n)||o===0){setText("pcChange","\u2014");return}var pct=(n-o)/Math.abs(o)*100;setText("pcChange",(pct>=0?"+":"")+fmt(pct,1)+"<small>%</small>")}}["pcOld","pcNew"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcPctChange),e.addEventListener("change",calcPctChange))}),calcPctChange();function gcd(a,b){return b?gcd(b,a%b):a}function calcFracToDecimal(){if(el("frFeet")){var feet=num("frFeet"),inches=num("frInches"),frac=parseFloat(el("frFraction").value);if(isNaN(feet)||isNaN(inches)){setText("frDecFeet","\u2014"),setText("frDecInches","\u2014"),setText("frMm","\u2014");return}var totalInches=feet*12+inches+frac,decFeet=totalInches/12,mm=totalInches*25.4;setText("frDecFeet",fmt(decFeet,4)+" <small>ft</small>"),setText("frDecInches",fmt(totalInches,4)+" in"),setText("frMm",fmt(mm,1)+" mm")}}["frFeet","frInches","frFraction"].forEach(function(id){var e=el(id);e&&(e.addEventListener("input",calcFracToDecimal),e.addEventListener("change",calcFracToDecimal))}),calcFracToDecimal();function calcDecimalToFrac(){if(el("frDecIn")){var decIn=num("frDecIn");if(isNaN(decIn)){setText("frNearest","\u2014");return}var sign=decIn<0?-1:1,absIn=Math.abs(decIn),sixteenths=Math.round(absIn*16),feet=Math.floor(sixteenths/192),remAfterFeet=sixteenths-feet*16*12,wholeInches=Math.floor(remAfterFeet/16),frac16=remAfterFeet%16,fracStr="";if(frac16>0){var g=gcd(frac16,16);fracStr=" "+frac16/g+"/"+16/g}var out=sign<0?"-":"";feet>0&&(out+=feet+"' "),out+=wholeInches+fracStr+'"',setText("frNearest",out)}}el("frDecIn")&&(el("frDecIn").addEventListener("input",calcDecimalToFrac),el("frDecIn").addEventListener("change",calcDecimalToFrac),calcDecimalToFrac());var BASE_RE={2:/^[01]+$/,8:/^[0-7]+$/,10:/^[0-9]+$/,16:/^[0-9a-fA-F]+$/};function fromBase(str,base){return str?BASE_RE[base].test(str)?parseInt(str,base):NaN:NaN}function updateBases(sourceId,sourceBase){var str=el(sourceId).value.trim(),val=fromBase(str,sourceBase),hint=el("nbHint");if(isNaN(val)){hint.textContent="Invalid characters for that base \u2014 check the value.",hint.style.color="var(--red)";return}hint.textContent="Type in any field \u2014 the other three update to match.",hint.style.color="",sourceId!=="nbDec"&&(el("nbDec").value=val.toString(10)),sourceId!=="nbHex"&&(el("nbHex").value=val.toString(16).toUpperCase()),sourceId!=="nbBin"&&(el("nbBin").value=val.toString(2)),sourceId!=="nbOct"&&(el("nbOct").value=val.toString(8))}el("nbDec")&&(el("nbDec").addEventListener("input",function(){updateBases("nbDec",10)}),el("nbHex").addEventListener("input",function(){updateBases("nbHex",16)}),el("nbBin").addEventListener("input",function(){updateBases("nbBin",2)}),el("nbOct").addEventListener("input",function(){updateBases("nbOct",8)}))})()}catch(e){console.error("app.js module #23 error:",e)}
